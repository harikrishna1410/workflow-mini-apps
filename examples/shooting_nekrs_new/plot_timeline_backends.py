...
def plot_timelines(logfiles, backend, labels=None, output=None):
    ...
    for idx, file in enumerate(logfiles):
        ...
        for t, dt, rel in zip(tstep_times, dt_times, rel_times):
            if dt > 0:
                ...
                ax.barh(idx, dt, left=rel-dt, height=0.6, 
    ...
    ax.set_title(f'Timeline ({backend})')
...
def main():
    ...
    for backend in ["filesystem","redis","nodelocal","dragon"]:
        try:
            logfiles = [os.path.join(args.basedir,f"logs_{backend}", f) for f in ["sim_0.log","train_ai_0.log"]]#,"infer_ai_0.log", ]]
            labels = ["Simulation", "Training"]#, "Inference"]
            plot_timelines(logfiles, backend, labels, f"all_timeline_plot_{backend}.png")
