...
def main(nsteps=10,write_freq=1,data_size=(32,32,32),dtype=np.float64,config={"type":"filesystem"}):
    # Initialize MPI
    comm = MPI.COMM_WORLD
    rank = comm.Get_rank()
    size = comm.Get_size()
    ...
    for i in range(nsteps):
        ...
        if i % write_freq == 0:
            ...
            while not simulation.poll_staged_data(f"ai_data_{rank}_{i//write_freq}"):
                # Wait for the AI to consume the data
                time.sleep(1)
            # Read the AI's inference result
            ai_result = simulation.stage_read(f"ai_data_{rank}_{i//write_freq}")
            #clean up the staged data
            simulation.clean_staged_data(f"ai_data_{rank}_{i//write_freq}")
    if rank == 0:
        simulation.clean()

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--nsteps", type=int, default=10, help="Number of simulation steps")
    ...
    main(nsteps=args.nsteps, write_freq=args.write_freq, data_size=tuple(args.data_size), dtype=dtype, config=config)
