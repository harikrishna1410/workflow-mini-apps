...
def test_run_mpi():
    cmd  = ["mpiexec", "-n", "2", "python3", "-c", 
            "'from mpi4py import MPI; from wfMiniAPI.simulation import Simulation; comm = MPI.COMM_WORLD; sim = Simulation(comm); sim.add_kernel(\"MatMulSimple2D\",run_count=10); sim.run()'"]
    ...
    if DPNP_AVAILABLE:
        cmd  = ["mpiexec", "-n", "2", "python3", "-c", 
            "'from mpi4py import MPI; from wfMiniAPI.simulation import Simulation; comm = MPI.COMM_WORLD; sim = Simulation(comm); sim.add_kernel(\"MatMulSimple2D\",device=\"xpu\",run_count=10); sim.run()'"]
        result = subprocess.run(" ".join(cmd), shell=True, capture_output=True)
        assert result.returncode == 0, f"Error running MPI simulation: {result.stderr.decode()}"
