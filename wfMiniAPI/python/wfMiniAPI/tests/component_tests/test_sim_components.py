...
def test_run_mpi():
    cmd  = ["mpiexec", "-n", "2", "python3", "-c", 
            "'from mpi4py import MPI; from wfMiniAPI.simulation import Simulation; comm = MPI.COMM_WORLD; sim = Simulation(comm); sim.add_kernel(\"MatMulSimple2D\"); sim.run(n_steps=10)'"]
