...
def test_ai_ddp():
    cmd = "mpirun -n 4 python3 -c " + f"'from mpi4py import MPI;from wfMiniAPI.training import AI;comm = MPI.COMM_WORLD;AI(num_hidden_layers=2,ddp=True,comm=comm).train(run_count=1)'"
    result = subprocess.run(cmd, shell=True, capture_output=True)
    assert result.returncode == 0, f"Distributed AI component failed to execute. Error: {result.stderr}"

    try:
        if torch.cuda.is_available():
            cmd = "mpirun -n 4 python3 -c " + f"'from mpi4py import MPI;from wfMiniAPI.training import AI;comm = MPI.COMM_WORLD;AI(num_hidden_layers=2,ddp=True,device=\"cuda\",comm=comm).train(run_count=1)'"
            result = subprocess.run(cmd, shell=True, capture_output=True)
            assert result.returncode == 0, f"Distributed AI component failed to execute. Error: {result.stderr}"
        else:
            print("CUDA is not available. Skipping GPU tests.")
        ...
        if not hasattr(torch, 'xpu'):
            ...
            return
        if torch.xpu.is_available():
            cmd = "mpirun -n 4 python3 -c " + f"'from mpi4py import MPI;from wfMiniAPI.training import AI;comm = MPI.COMM_WORLD;AI(num_hidden_layers=2,ddp=True,device=\"xpu\",comm=comm).train(run_count=1)'"
            result = subprocess.run(cmd, shell=True, capture_output=True)
            assert result.returncode == 0, f"Distributed AI component failed to execute. Error: {result.stderr}"
        else:
            print("Intel XPU is not available. Skipping XPU tests.")
